...
local AttackUnitSpawnWaitTime = 0.5 * 1000 -- 0.5s (ms)
...
local function getDifficultyNumber()
...
function GameModeSpawnCooldown()
	...
	local attackerBuyWindow = 1000 * 10 -- Bot has x seconds to buy units from source. Do not change.

    if botRole == "attacker" then
        local adjusted = attackerBuyWindow / difficultyMultiplier
        return adjusted
    end
...
function GetCurrentSpawnWaitTime()
    if botRole == "attacker" then
        return AttackUnitSpawnWaitTime
    else
...
function GetUnitToSpawn(units)
 ...
 -- Choose appropriate UnitSpawnWaitTime based on Attacker vs Defender
...
function OnGameStart()
    setBotRole()
    OnGameStartUtility(BotApi.Instance.unitMode,BotApi.Instance.army)
end
